...
try:
    from lib.plugin_common import get_points, online_score_xml
    from lib.version import __version__
except (ImportError, ModuleNotFoundError):
    from renfield.lib.plugin_common import get_points, online_score_xml
...
def show_mults(self):
    ...
    if im_dutch(self) is False:
        sql = f"select count(DISTINCT(NR || ':' || Band || ':' || Mode)) as mult_count from dxlog where ContestName = '{self.database.current_contest}' and NR in ('DR','FL','FR','GD','GR','LB','NB','NH','OV','UT','ZH','ZL');"
    else:
        sql = f"select count(DISTINCT(CountryPrefix || ':' || Band || ':' || Mode)) as mult_count from dxlog where ContestName = '{self.database.current_contest}';"
