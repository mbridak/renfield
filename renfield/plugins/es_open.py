...
try:
    from lib.ham_utility import calculate_wpx_prefix
...
def specific_contest_check_dupe(self, call):
    ...
    iso_current_time = datetime.datetime.now(datetime.UTC)
...
def show_mults(self, rtc=None):
    ...
    query = f"SELECT count(DISTINCT(substr(WPXPrefix,3,1) || ':' || Band || ':' || Mode)) as mults from DXLOG where ContestName = '{self.database.current_contest}' AND CountryPrefix = 'ES' AND WPXPrefix != '{our_prefix}';"
