...
def show_mults(self, rtc=None) -> int:
    ...
    sql = (
        ...
        f"from dxlog where CountryPrefix in ('K', 'VE', 'VK', 'JA') and ContestName = '{self.database.current_contest}';"
    )
    result = self.database.exec_sql(sql)
    if result:
    ...
    sql = (
        ...
        f"from dxlog where Call = 'EA4URE' and ContestName = '{self.database.current_contest}';"
    )
    result = self.database.exec_sql(sql)
    if result:
